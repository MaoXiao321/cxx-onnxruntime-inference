...
TextDetector::TextDetector(string path)
{
	...
	net = new Session(env, (const char*)model_path.c_str(), sessionOptions);
...
Mat TextDetector::preprocess(Mat srcimg)
{
	...
	int h = srcimg.rows;
	int w = srcimg.cols;
	cout << "origin size: h:" << h << ", w:" << w << endl;
	float scale_h = 1;
	float scale_w = 1;
	if (h < w)
	{
		scale_h = (float)this->short_size / (float)h; // 缩小幅度比较轻
		float tar_w = (float)w * scale_h;
		tar_w = tar_w - (int)tar_w % 32; //保证被32整除
	...
	cout << "det size: h:" << dstimg.rows << ", w:" << dstimg.cols << endl;
	return dstimg;
}
...
vector< vector<Point2f> > TextDetector::detect(Mat& srcimg)
{
	...
	memcpy(binary.data, floatArray, outputCount * sizeof(float)); //从floatArray向binary拷贝指定数量的内容

	// Threshold
	Mat bitmap;
	threshold(binary, bitmap, binaryThreshold, 255, THRESH_BINARY); //图像二值化，THRESH_BINARY表示值大于阈值时设为最大值255
	// Scale ratio，原图到最后输出图的缩小比例
	...
	size_t numCandidate = min(contours.size(), (size_t)(maxCandidates > 0 ? maxCandidates : INT_MAX)); //min(轮廓点，INT_MAX)
	...
	for (size_t i = 0; i < numCandidate; i++)
	{
		...
		// Unclip
		RotatedRect box = minAreaRect(contourScaled); // 返回包覆输入信息的最小斜矩形
...
void TextDetector::draw_pred(Mat& srcimg, vector< vector<Point2f> > results, vector<string> labels)
{
	for (int i = 0; i < results.size(); i++)
	{
		for (int j = 0; j < 4; j++)
		{
			...
			if (j < 3)
			{
				line(srcimg, Point((int)results[i][j].x, (int)results[i][j].y), Point((int)results[i][j + 1].x, (int)results[i][j + 1].y), Scalar(255, 0, 0), 5);
			}
			else
			{
				line(srcimg, Point((int)results[i][j].x, (int)results[i][j].y), Point((int)results[i][0].x, (int)results[i][0].y), Scalar(255, 0, 0),5);
			}
		}
		string label = labels[i];
		putText(srcimg, label, Point((int)results[i][0].x, (int)results[i][0].y - 5), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(255, 0, 0), 2);
		// Draw::drawText(srcimg, label, 0.3, Scalar(0, 255, 0), Scalar(0, 255, 0), Point((int)results[i][0].x, (int)results[i][0].y - 5));
	}
}

float TextDetector::contourScore(const Mat& binary, const vector<Point>& contour)
{
	Rect rect = boundingRect(contour); //计算轮廓的垂直边界最小矩形
